import { config,validToken,confirm } from '@/lib/waitlist/service';
...
export async function POST(request:Request){
 ...
 try{const ok=await confirm(body.token);return ok?reply({success:true}):reply({error:'This link has expired or the request was removed. Please join again.'},410);}catch{return reply({error:'We could not complete this request. Please retry.'},503);}
}
